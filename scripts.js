document.addEventListener('DOMContentLoaded', function() {
    ...
    // const sortScheduleButton = document.getElementById('sort-schedule'); //暫時不用調整順序
    ...
    fetch('routes.xlsx')
        ...
        .then(data => {
            ...
            stationSelect.addEventListener('change', () => {
                ...
                //若沒有選擇工作表，則清空'routeSelect'和'scheduleContainer'的內容
                } else {
                    routeSelect.innerHTML = '';
    ...
    function displaySchedules(filter) {
        ...
        //根據'filter'所過濾的schedules數據（其中的值包括'workday','holiday','all'）
        ...
        if (schedule) {
            ...
            //根據'schedule[5]'的值
                //'dayTypeSelect.value'：將下方的運算符結果設置在'dayTypeSelect'的值裡
                    //'schedule[5] === 1'：判斷'schedule'中第六個元素是否為'1'
                    //若是true，則是'workday'
                    //若是false，則是'holiday'
            dayTypeSelect.value = schedule[5] === 1 ? 'workday' : 'holiday';
    ...
    function saveSchedule() {
        ...
        //下方為'newSchedule'的陣列內容
            ...
            //檢查用戶是否選擇了'workday'，若是結果會是'1'；反之會是'0'
            dayTypeSelect.value === 'workday' ? 1 : 0
        ];

        //用作檢查'schedules'是否與'editingScheduleIndex'一樣
            //檢查'editingScheduleIndex'是否為null'
        if (editingScheduleIndex !== null) {
            ...
            //目的是根據'editingScheduleIndex'是否為'null'來決定要「更新班次」還是「新增班次」
                //若找到'scheduleIndex'不是'-1'，代表在陣列中找到符合的'editingScheduleIndex'，意味著用戶正在編輯一個現有的班次
            if (scheduleIndex !== -1) {
    ...
    //     const isSelected = selectedSchedules.length > 0;
    //     editScheduleButton.style.display = selectedSchedules.length === 1 ? 'inline-block' : 'none';
    //     deleteScheduleButton.style.display = isSelected ? 'inline-block' : 'none';
    // }

    //切換排序順序//
    // function toggleSortOrder() {
    //     currentSortOrder = currentSortOrder === 'asc' ? 'desc' : 'asc'; //asc（升序）、desc（降序）
    //     displaySchedules('all');
    // }
    ...
    //排序班次的按鈕
    // sortScheduleButton.addEventListener('click', toggleSortOrder); //暫時不用調整順序
    //刪除班次的按鈕
